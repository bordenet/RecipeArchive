...
Submit diagnostic data for parsing failures and errors (no authentication required).
...
- `sortBy` (optional): `created` | `updated` | `title` | `prepTime` | `cookTime` | `servings` (default: `created`)
...
- Start with `POST /recipes` (recipe creation)
- Then `GET /recipes` (list recipes)
- Add `GET /recipes/{id}` (single recipe)
