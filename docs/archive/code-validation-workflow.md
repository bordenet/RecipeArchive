...
1. **Pre-Edit Checks**
   ...
   - Run `go run tools/monorepo-validator --p1` to check for linting errors
   - Document all issues that need to be fixed

2. **Edit Process**
   - Make one focused change at a time
   - After EACH edit:
     - Run `go run tools/monorepo-validator --p1` to verify no new issues introduced
...
3. **Post-Edit Validation**
   - Run `go run tools/monorepo-validator --med` on ALL affected files
...
1. Always run `go run tools/monorepo-validator --p1` before claiming success
