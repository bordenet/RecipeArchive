...
```bash
# Full verification using the new Go tool (runs medium tier by default)
go run tools/monorepo-validator
...
```bash
# Run all tests using the new Go tool
go run tools/monorepo-validator --all
...
1. Verify setup: `go run tools/monorepo-validator`
