...
- **Per-User Limits**: Max 20 normalizations per day per user
- **Global Limits**: Max 1000 API calls per day (adjustable)
...
**Parallel Processing:**
```typescript
async function processRecipeBatch(recipes: RecipeData[]): Promise<ProcessedRecipe[]> {
