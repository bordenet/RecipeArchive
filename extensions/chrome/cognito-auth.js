...
class CognitoAuth {
    ...
    try {
      ...
      if (
        ...
        return {
          success: false,
          error:
            "Direct password authentication not enabled. Please use OAuth2 flow or enable USER_PASSWORD_AUTH in your Cognito User Pool.",
          suggestOAuth2: true,
        };
      }
  ...
  _getCognitoDomain() {
    ...
    throw new Error(
      "Cognito domain not configured. OAuth2 requires a hosted UI domain."
    );
  }
  ...
  async _launchWebAuthFlow(url) {
    ...
    if (
      extensionAPI &&
      extensionAPI.identity &&
      extensionAPI.identity.launchWebAuthFlow
    ) {
  ...
  async _isTokenValid() {
    try {
      ...
      const validation = jwtValidator.validateCognitoAccessToken(
        accessToken,
        this.clientId
      );
