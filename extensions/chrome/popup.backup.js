...
document.addEventListener('DOMContentLoaded', async function initializePopup() {
...
async function checkAuthStatus() {
  ...
  try {
    ...
    if (!config || typeof config.getCognitoConfig !== 'function') {
      const errorMsg = 'CONFIG getCognitoConfig method not available';
      console.error('RecipeArchive Safari:', errorMsg);
    ...
    if (!cognitoConfig) {
      const errorMsg = 'Failed to get Cognito configuration';
      console.error('RecipeArchive Safari:', errorMsg);
      showMessage(errorMsg, 'error');
      return;
    }
    ...
    if (typeof SafariCognitoAuth === 'undefined') {
      const errorMsg = 'SafariCognitoAuth not loaded - check cognito-auth.js script tag';
      console.error('RecipeArchive Safari:', errorMsg);
      showMessage(errorMsg, 'error');
      return;
    }
...
function showAuthRequired() {
  ...
  domElements.authButton.onclick = async function handleAuthClick() {
...
function showMainInterface(username) {
  ...
  domElements.logoutButton.onclick = async function handleLogout() {
    try {
      // Defensive check for CONFIG availability
      if (typeof CONFIG === 'undefined' || !CONFIG.getCognitoConfig) {
        console.error('CONFIG not available for logout');
        showMessage('Configuration error during logout', 'error');
        return;
      }
...
async function sendRecipeToBackend(recipe) {
  ...
  try {
    ...
    const config = window.RecipeArchiveConfig;
    if (!config || typeof config.getCurrentAPI !== 'function' || typeof config.getCognitoConfig !== 'function') {
