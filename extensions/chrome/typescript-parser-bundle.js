...
(() => {
  ...
  function getEscaper2(regex, map2) {
    return function escape2(data2) {
      ...
      while ((match = regex.exec(data2)) !== null) {
        ...
        const next2 = map2.get(char);
        if (next2 !== void 0) {
          ret += data2.substring(lastIdx, i) + next2;
          lastIdx = i + 1;
        } else {
          ret += `${data2.substring(lastIdx, i)}&#x${getCodePoint(data2, i).toString(16)};`;
          lastIdx = regex.lastIndex += Number((char & 64512) === 55296);
        }
      }
      return ret + data2.substr(lastIdx);
    };
  }
        result += map2.get(match[0].charCodeAt(0));
  ...
  var BaseParser = class _BaseParser {
    constructor() {
      if (this.constructor === _BaseParser) {
        ...
        try {
          ...
          if (Array.isArray(jsonData)) {
            const recipe = jsonData.find(
              (item) => item && typeof item === "object" && isRecipeType(item["@type"])
            );
    ...
    extractElements(doc, selectors) {
      ...
      for (const selector of selectors) {
        try {
          ...
          if (elements.length) {
            return elements.map((el) => this.sanitizeText(el.textContent)).filter((text3) => text3.length > 0);
          }
        } catch (err) {
        }
      }
  ...
  // ../../parsers/sites/smitten-kitchen.ts
  var SmittenKitchenParser = class extends BaseParser {
    canParse(url) {
    ...
    async parse(html3, url) {
      ...
      if (jsonLd) {
        ...
        let title = this.sanitizeText($2(".jetpack-recipe-title, h1.entry-title, h1.post-title, h1").first().text() || $2("h1").first().text() || "");
        let author = this.sanitizeText($2(".jetpack-recipe-source, p.recipe-meta + p, .author-meta, .author, .byline .author").first().text().replace(/Source:\s*|Author:\s*/gi, "").trim() || "Deb Perelman");
        let ingredients = [];
        $2(".jetpack-recipe-ingredient").each((_, el) => {
          const text3 = $2(el).text().trim();
          if (text3) ingredients.push({ text: this.sanitizeText(text3) });
        });
        if (ingredients.length === 0) {
          ingredients = $2(".recipe-ingredients li, .ingredients li, .ingredient, .wprm-recipe-ingredient, ul li, .entry-content ul li").map((_, el) => ({ text: this.sanitizeText($2(el).text()) })).get();
        }
        if (ingredients.length === 0) {
          $2('h2:contains("Ingredients")').nextAll("ul").each((_, ul) => {
            $2(ul).find("li").each((__, el) => {
              const text3 = $2(el).text().trim();
        ...
        if (ingredients.length === 0) {
          const entryText = $2(".entry-content").text();
          const cocktailMatch = entryText.match(/(\d+\/?\d*\s*(cup|fluid ounces|oz|ml)\s+bourbon)[\s\S]*?(\d+\/?\d*\s*(cup|fluid ounces|oz|ml)\s+amaro nonino)[\s\S]*?(\d+\/?\d*\s*(cup|fluid ounces|oz|ml)\s+Aperol)[\s\S]*?(\d+\/?\d*\s*(cup|fluid ounces|oz|ml)\s+lemon juice)/i);
          if (cocktailMatch) {
            ingredients = [
              { text: cocktailMatch[1] },
              { text: cocktailMatch[3] },
              { text: cocktailMatch[5] },
              { text: cocktailMatch[7] }
            ];
          } else {
            const lines = entryText.split(/\n|\r/).map((l) => l.trim()).filter((l) => l.length > 0);
            const keys = ["bourbon", "amaro", "aperol", "lemon juice"];
            for (const key of keys) {
              const line = lines.find((l) => l.toLowerCase().includes(key));
              if (line) ingredients.push({ text: line });
            }
          }
        }
        ...
        if (ingredientP.length) {
          ...
          if (raw) {
            ingredients = raw.split(/<br\s*\/>/i).map((t) => ({ text: this.sanitizeText($2(t).text() || $2("<div>" + t + "</div>").text()) })).filter((i) => i.text);
          }
        }
        let instructions = [];
        ...
        if (instructions.length === 0) {
          $2('h2:contains("Directions"), h2:contains("Instructions")').nextAll("ul").each((ulIdx, ul) => {
            $2(ul).find("li").each((liIdx, el) => {
              const text3 = $2(el).text().trim();
              if (text3) instructions.push({ stepNumber: instructions.length + 1, text: text3 });
            });
          });
        }
        if (instructions.length === 0) {
          const entryText = $2(".entry-content").text();
          const instrMatch = entryText.match(/mix everything in a jar[\s\S]*?freezer[\s\S]*?overnight[\s\S]*?serve[\s\S]*?stir mixture[\s\S]*?pour into/i);
          if (instrMatch) {
            instructions.push({ stepNumber: 1, text: this.sanitizeText(instrMatch[0]) });
          } else {
            const lines = entryText.split(/\n|\r/).map((l) => l.trim()).filter((l) => l.length > 0);
            const instrLines = lines.filter((l) => /(mix|freeze|serve|stir|pour)/i.test(l));
            if (instrLines.length) {
              instructions.push({ stepNumber: 1, text: instrLines.join(" ") });
            }
          }
        }
        ...
        entryContent.find("p").slice(instrIdx + 1).each((i, el) => {
          ...
          if (html4 && (html4.includes("Preheat oven") || html4.match(/\bBake\b|\bAllow to cool\b|\bDo ahead\b|\bFlouring\b|\bSift together\b|\bPour into\b|\bMelt together\b|\bStir\b|\bAdd to\b|\bToss the walnuts\b|\bDo not overbake\b/))) {
            instructions.push({ stepNumber: instructions.length + 1, text: this.sanitizeText($2(el).text()) });
          }
        });
      ...
      if (!recipe.title || !recipe.source || !recipe.ingredients.length || !recipe.instructions.length) {
  ...
  // ../../parsers/sites/food-network.ts
  var FoodNetworkParser = class extends BaseParser {
    canParse(url) {
    ...
    async parse(html3, url) {
      ...
      if (jsonLd) {
        ...
        const validation2 = this.validateRecipe(recipe2);
        if (validation2.isValid) {
          return recipe2;
        }
      }
      const title = this.sanitizeText([
        $2("h1.o-AssetTitle__a-HeadlineText").text(),
        $2("h1.recipe-title").text(),
        $2("h1").text()
      ].find((t) => t && t.trim().length > 0) || "");
      const author = this.sanitizeText([
        $2(".o-Attribution__a-Name").text(),
        $2(".recipe-author").text(),
        $2(".chef-name").text()
      ].find((a) => a && a.trim().length > 0) || "Food Network");
      let ingredients = [];
      $2(".o-Ingredients__a-Ingredient, .ingredients-list__item, .ingredient, .recipe-ingredients li, ul li, .entry-content ul li").each((_, el) => {
        let label = $2(el).find(".o-Ingredients__a-Ingredient--CheckboxLabel").text().trim();
        if (!label) label = $2(el).text().trim();
      ...
      for (const selector of instructionSelectors) {
        $2(selector).each((_, el) => {
          const text3 = this.sanitizeText($2(el).text());
          if (text3 && text3.length > 0) instructionsRaw.push(text3);
        });
      }
      let instructions = instructionsRaw.filter((text3) => typeof text3 === "string" && text3.trim().length > 0).map((text3, idx) => ({ stepNumber: idx + 1, text: text3 }));
      if (instructions.length === 0) {
        $2('h2:contains("Directions"), h2:contains("Instructions")').nextAll("ul").each((ulIdx, ul) => {
          $2(ul).find("li").each((liIdx, el) => {
            const text3 = $2(el).text().trim();
  ...
  // ../../parsers/sites/nyt-cooking.ts
  var NYTCookingParser = class extends BaseParser {
    canParse(url) {
    ...
    async parse(html3, url) {
      ...
      const title = this.sanitizeText($2('h1.recipe-title, h1[data-testid="recipe-title"], h1.pantry-recipe-title, h1').first().text() || "");
      const author = this.sanitizeText($2('.recipe-author, [data-testid="recipe-author"], .byline-author, .author').first().text() || "NYT Cooking");
      const ingredients = $2('[data-testid="IngredientList"] li, .recipe-ingredients li, .ingredients-section li, [data-module="Ingredients"] li, ul[data-testid="ingredients"] li').map((_, el) => ({ text: this.sanitizeText($2(el).text()) })).get();
      const instructions = $2('[data-testid="MethodList"] li, .recipe-instructions li, .instructions-section li, [data-module="Instructions"] li, ol[data-testid="instructions"] li').map((_, el) => ({ stepNumber: _ + 1, text: this.sanitizeText($2(el).text()) })).get();
      const imageUrl = $2('.recipe-photo img, [data-testid="recipe-image"] img, img').first().attr("src") || void 0;
      const recipe = {
        title,
        source: url,
        author,
  ...
  var AllRecipesParser = class extends BaseParser {
    ...
    async parse(html3, url) {
      const $2 = load(html3);
      const canonicalUrl = $2('link[rel="canonical"]').attr("href");
      const ogUrl = $2('meta[property="og:url"]').attr("content");
      ...
      if (jsonLd) {
        ...
        const validation = this.validateRecipe(recipe2);
        if (validation.isValid) {
          if (recipe2.title && recipe2.ingredients.length > 0 && recipe2.instructions.length > 0) {
            return recipe2;
          }
        }
      }
      ...
      const author = this.sanitizeText(
        $2(".recipe-author, .by-author, .author-name").first().text().replace(/^by\s*/i, "") || ""
      );
      let ingredients = [];
      ...
      for (const selector of ingredientSelectors) {
        const found = $2(selector).map((_, el) => {
          const text3 = this.sanitizeText($2(el).text());
          return text3 && text3.length > 0 ? { text: text3 } : null;
        }).get().filter(Boolean);
        if (found.length > 0) {
          ingredients = found;
          break;
      ...
      for (const selector of instructionSelectors) {
        const found = $2(selector).map((i, el) => {
          const text3 = this.sanitizeText($2(el).text());
          return text3 && text3.length > 0 ? { stepNumber: i + 1, text: text3 } : null;
        }).get().filter(Boolean);
        if (found.length > 0) {
          instructions = found;
          break;
  ...
  // ../../parsers/sites/loveandlemons.ts
  var LoveAndLemonsParser = class extends BaseParser {
    ...
    async parse(html3, url) {
      ...
      const instructions = $2(".wprm-recipe-instruction-text").map((_, el) => ({ stepNumber: _ + 1, text: this.sanitizeText($2(el).text()) })).get();
      const recipe = {
        title,
        source: url,
        ingredients,
      ...
      if (jsonLd) {
        ...
        if (jsonLd.keywords) {
          if (Array.isArray(jsonLd.keywords)) {
            tags = jsonLd.keywords.map((k) => this.sanitizeText(k));
        ...
        if (jsonLd.recipeCategory) {
          ...
          if (Array.isArray(jsonLd.recipeCategory)) {
            tags.push(...jsonLd.recipeCategory.map((c) => this.sanitizeText(c)));
          } else if (typeof jsonLd.recipeCategory === "string") {
            tags.push(...jsonLd.recipeCategory.split(/,|;/).map((c) => this.sanitizeText(c)).filter(Boolean));
          }
        }
        recipe = {
          title: this.sanitizeText(jsonLd.name),
        ...
        let tags = void 0;
        const tagEls = $2('span.text-approved, .tags, meta[property="og:keywords"]');
        if (tagEls.length) {
          tags = [];
          tagEls.each((_, el) => {
  ...
  var EpicuriousParser = class extends BaseParser {
    ...
    async parse(html3, url) {
      ...
      const title = this.sanitizeText(
        $2("h1.recipe-hed, h1").first().text() || ""
      );
      const author = this.sanitizeText(
        $2('.author-name, .by-author, [data-testid="BylineWrapper"]').first().text().replace(/^by\s*/i, "") || ""
      );
      let ingredients = [];
      const ingredientSelectors = [
      ...
      let imageUrl = $2(".recipe-header-image img, .recipe-image img, .hero-image img").first().attr("src");
      if (!imageUrl) {
        imageUrl = $2('meta[property="og:image"]').attr("content");
      }
      const prepTime = this.sanitizeText($2('.prep-time, [data-testid="PrepTime"]').first().text());
      const cookTime = this.sanitizeText($2('.cook-time, [data-testid="CookTime"]').first().text());
      const totalTime = this.sanitizeText($2('.total-time, [data-testid="TotalTime"]').first().text());
      const servings = this.sanitizeText($2('.servings, .recipe-yield, [data-testid="Yield"]').first().text());
      const recipe = {
        title,
        source: url,
        author: author || void 0,
  ...
  // ../../parsers/sites/washington-post.ts
  var WashingtonPostParser = class extends BaseParser {
    canParse(url) {
      return url.includes("washingtonpost.com/food") || url.includes("washingtonpost.com") && url.includes("recipe");
    }
    async parse(html3, url) {
      const jsonLd = this.extractJsonLD(html3);
      if (jsonLd) {
      ...
      const $2 = load(html3);
      const title = this.extractTitle($2);
      const ingredients = this.extractIngredients($2);
      let instructions = this.extractInstructions($2);
      const imageUrl = this.extractImage($2);
      const metadata = this.extractMetadata($2);
      if (instructions.length === 0) {
    ...
    extractTitle($2) {
      ...
      for (const selector of titleSelectors) {
        const title = $2(selector).first().text().trim();
        if (title) {
    ...
    extractIngredients($2) {
      ...
      for (const selector of ingredientSelectors) {
        $2(selector).each((_, el) => {
          const text3 = $2(el).text().trim();
          if (text3 && !this.isIngredientHeader(text3)) {
            ingredients.push({ text: this.sanitizeText(text3) });
          }
        });
        if (ingredients.length > 0) break;
      }
      if (ingredients.length === 0) {
        ingredients.push(...this.extractIngredientsFromParagraphs($2));
      }
      return ingredients;
    }
    extractInstructions($2) {
      const instructions = [];
      const instructionSelectors = [
        ".recipe-instructions li",
      ...
      for (const selector of instructionSelectors) {
        $2(selector).each((idx, el) => {
          const text3 = $2(el).text().trim();
          if (text3 && !this.isInstructionHeader(text3)) {
            instructions.push({
              stepNumber: idx + 1,
      ...
      if (instructions.length === 0) {
        instructions.push(...this.extractInstructionsFromParagraphs($2));
      }
      return instructions;
    }
    extractImage($2) {
      ...
      for (const selector of imageSelectors) {
        if (selector.startsWith("meta")) {
          const content = $2(selector).attr("content");
          if (content) return content;
        } else {
          const src = $2(selector).first().attr("src") || $2(selector).first().attr("data-src");
    ...
    extractMetadata($2) {
      const metadata = {};
      const prepTime = $2('[data-qa="prep-time"], .prep-time').text().trim();
      const cookTime = $2('[data-qa="cook-time"], .cook-time').text().trim();
      const totalTime = $2('[data-qa="total-time"], .total-time').text().trim();
      if (prepTime) metadata.prepTime = prepTime;
      if (cookTime) metadata.cookTime = cookTime;
      if (totalTime) metadata.totalTime = totalTime;
      const servings = $2('[data-qa="servings"], .servings, .yield').text().trim();
      if (servings) metadata.servings = servings;
      const description = $2(".recipe-description, .article-summary, .dek").first().text().trim();
      if (description) {
        metadata.notes = [this.sanitizeText(description)];
      }
      return metadata;
    }
    isIngredientHeader(text3) {
      const headers = ["ingredients", "for the", "you will need", "shopping list"];
      return headers.some((header) => text3.toLowerCase().includes(header));
    }
    isInstructionHeader(text3) {
      const headers = ["instructions", "method", "directions", "steps"];
      return headers.some((header) => text3.toLowerCase().includes(header));
    }
    extractIngredientsFromParagraphs($2) {
      const ingredients = [];
      $2("p").each((_, el) => {
        const text3 = $2(el).text().trim();
        if (this.looksLikeIngredient(text3)) {
          ingredients.push({ text: this.sanitizeText(text3) });
        }
      });
      return ingredients;
    }
    extractInstructionsFromParagraphs($2) {
      const instructions = [];
      $2("p").each((idx, el) => {
        const text3 = $2(el).text().trim();
        if (this.looksLikeInstruction(text3)) {
          instructions.push({
    ...
    looksLikeIngredient(text3) {
      ...
      return ingredientIndicators.some((pattern) => pattern.test(text3)) && text3.length < 200;
    }
    looksLikeInstruction(text3) {
      ...
      return instructionIndicators.some((pattern) => pattern.test(text3)) && text3.length > 20 && text3.length < 500;
    }
    parseFromJsonLD(jsonLd, url) {
      return {
        ...
        instructions: (jsonLd.recipeInstructions || []).map((instruction, idx) => ({
          stepNumber: idx + 1,
          text: typeof instruction === "string" ? this.sanitizeText(instruction) : this.sanitizeText(instruction.text)
        })),
  ...
  // ../../parsers/sites/food-and-wine.ts
  var FoodAndWineParser = class extends BaseParser {
    ...
    async parse(html3, url) {
      ...
      const prepTime = this.sanitizeText($2('.prep-time, .recipe-prep-time, [itemprop="prepTime"]').first().text());
      const cookTime = this.sanitizeText($2('.cook-time, .recipe-cook-time, [itemprop="cookTime"]').first().text());
      const totalTime = this.sanitizeText($2('.total-time, .recipe-total-time, [itemprop="totalTime"]').first().text());
      const servings = this.sanitizeText($2('.servings, .recipe-servings, .recipe-yield, [itemprop="recipeYield"]').first().text());
      const recipe = {
        title,
        source: url,
        author: author || void 0,
  ...
  // ../../parsers/sites/damn-delicious.ts
  var DamnDeliciousParser = class extends BaseParser {
    ...
    async parse(html3, url) {
      ...
      let imageUrl = $2(".recipe-card-image img, .post-thumbnail img, .wp-post-image").first().attr("src");
      if (!imageUrl) {
        imageUrl = $2('meta[property="og:image"]').attr("content");
      }
      const prepTime = this.sanitizeText($2('.recipe-card-prep-time, .prep-time, [itemprop="prepTime"]').first().text());
      const cookTime = this.sanitizeText($2('.recipe-card-cook-time, .cook-time, [itemprop="cookTime"]').first().text());
      const totalTime = this.sanitizeText($2('.recipe-card-total-time, .total-time, [itemprop="totalTime"]').first().text());
      const servings = this.sanitizeText($2('.recipe-card-servings, .servings, [itemprop="recipeYield"]').first().text());
      const recipe = {
        title,
        source: url,
        author,
  ...
  var SeriousEatsParser = class extends BaseParser {
    ...
    async parse(html3, url) {
      ...
      const prepTime = this.sanitizeText($2('.recipe-prep-time, .prep-time, [data-prep-time], [itemprop="prepTime"]').first().text());
      const cookTime = this.sanitizeText($2('.recipe-cook-time, .cook-time, [data-cook-time], [itemprop="cookTime"]').first().text());
      const totalTime = this.sanitizeText($2('.recipe-total-time, .total-time, [data-total-time], [itemprop="totalTime"]').first().text());
      const servings = this.sanitizeText($2('.recipe-servings, .servings, .recipe-yield, [data-servings], [itemprop="recipeYield"]').first().text());
      const notes = [];
