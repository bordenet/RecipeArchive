...
document.addEventListener("DOMContentLoaded", function () {
  ...
  container.style.cssText =
    "padding: 20px; min-width: 320px; font-family: -apple-system, BlinkMacSystemFont, \"Segoe UI\", Roboto, sans-serif;";
  ...
  try {
    ...
    if (
      ...
      if (retryCount < MAX_INIT_RETRIES) {
        console.log(
          `⏳ Waiting for Safari dependencies to load (attempt ${retryCount + 1}/${MAX_INIT_RETRIES})`
        );
        setTimeout(() => {
...
async function checkSiteSupportSafari(captureBtn, retryCount = 0) {
  ...
  try {
    ...
    if (typeof window.RecipeArchiveSites !== "undefined") {
      ...
      console.log(
        `⏳ Waiting for RecipeArchiveSites to load (Safari attempt ${retryCount + 1}/${MAX_RETRIES})`
      );
      setTimeout(() => {
      ...
      console.warn(
        "⚠️ RecipeArchiveSites failed to load after max retries (Safari)"
      );
...
function updateCaptureButtonSafari(
  captureBtn,
  isSupported,
  errorMessage = null
) {
...
async function downloadAndUploadImage(imageUrl, recipeTitle) {
  try {
    ...
    console.error("❌ Image download/upload failed:", _error);
    ...
    reportDiagnostic("image-processing-error", _error, {
      imageUrl: imageUrl,
      recipeTitle: recipeTitle,
...
async function isValidRecipePage(recipeData, pageUrl) {
  ...
  const ingredientsCount = (recipeData.ingredients || []).filter(
    (i) =>
      ...
      (i && i.items && Array.isArray(i.items) && i.items.length > 0)
  ).length;
  ...
  if (ingredientsCount === 0) {
    ...
    // Submit diagnostic data for parsing failure
    if (typeof submitDiagnosticData === "function") {
      await submitDiagnosticData({
        error: "No ingredients found",
        url: pageUrl,
        recipeData: recipeData,
        context: "Safari extension validation failure - ingredients count: 0"
      });
    }
  ...
  if (instructionsCount === 0) {
    ...
    if (typeof submitDiagnosticData === "function") {
      await submitDiagnosticData({
        error: "No cooking instructions found",
        url: pageUrl,
        recipeData: recipeData,
        context: "Safari extension validation failure - instructions count: 0"
      });
    }
  ...
  if (ingredientsCount < 2) {
    ...
    if (typeof submitDiagnosticData === "function") {
      await submitDiagnosticData({
        error: "Insufficient ingredients found",
        url: pageUrl,
        recipeData: recipeData,
        context: `Safari extension validation failure - ingredients count: ${ingredientsCount}`
      });
    }
  ...
  if (instructionsCount < 2) {
    ...
    if (typeof submitDiagnosticData === "function") {
      await submitDiagnosticData({
        error: "Insufficient instructions found",
        url: pageUrl,
        recipeData: recipeData,
        context: `Safari extension validation failure - instructions count: ${instructionsCount}`
      });
    }
  ...
  extensionAPI.tabs.query(
    ...
    async function (tabs) {
      ...
      if (
        ...
        !window.RecipeArchiveSites.isSupportedSite(currentTab.url)
      ) {
        ...
        try {
          const pingResponse = await new Promise((resolve, reject) => {
            ...
            extensionAPI.tabs.sendMessage(
              ...
              (response) => {
                console.log(
                  "🔧 Ping sendMessage callback - response:",
                  response
                );
                console.log(
                  "🔧 Ping sendMessage callback - lastError:",
          ...
          if (pingResponse && pingResponse.status === "pong") {
            console.log(
              "✅ Content script already loaded, sending capture message"
            );
            await sendCaptureMessage(tab.id, tab.url);
          } else {
            console.log(
              "🔧 Safari Web Extensions: Trying direct recipe capture"
            );
          ...
          clearTimeout(pingTimeout);
          console.log(
            "🔧 Safari Web Extensions: Trying direct recipe capture as fallback"
          );
          await captureRecipeDirectly(tab.id, tab.url);
        }
      } catch (error) {
...
async function captureRecipeDirectly(tabId, tabUrl = "unknown") {
  ...
  try {
      ...
      func: function () {
        ...
        function extractRecipeFromPage() {
          ...
          if (url.includes("foodnetwork.com")) {
            ...
            if (
              foodNetworkRecipe &&
              foodNetworkRecipe.ingredients &&
              foodNetworkRecipe.ingredients.length > 0
            ) {
        ...
        function extractFoodNetworkRecipe() {
          ...
          const title =
            document
              .querySelector("h1.o-AssetTitle__a-HeadlineText, h1")
          ...
          console.log(
            `🔍 Found ${ingredients.length} ingredients and ${steps.length} steps`
          );
        ...
        function extractGenericRecipe() {
          ...
          const title =
            document
              .querySelector(
                "h1[class*=\"recipe\"], .recipe-title, .entry-title, h1"
              )
              ?.textContent?.trim() || document.title;
          ...
          console.log(
            `🔍 Generic extraction found ${ingredients.length} ingredients and ${steps.length} steps`
          );
        ...
        function extractRecipeFromJsonLd() {
          ...
          for (const script of jsonLdScripts) {
            try {
              ...
              console.log("JSON-LD parsing failed:", _e.message);
            }
          }
          return null;
        }
        ...
        function extractSmittenKitchenRecipe() {
          ...
          const title =
            document.querySelector(".entry-title, h1")?.textContent?.trim() ||
            document.title;

          // Extract ingredients
          const ingredientElements = document.querySelectorAll(
            ".recipe-ingredients li, .recipe-summary ul li"
          );
          const ingredients = Array.from(ingredientElements)
            .map((el) => el.textContent?.trim())
            .filter((text) => text && text.length > 2);
          ...
          // Extract steps
          const stepElements = document.querySelectorAll(
            ".recipe-instructions li, .recipe-instructions ol li"
          );
          const steps = Array.from(stepElements)
            .map((el) => el.textContent?.trim())
            .filter((text) => text && text.length > 10);
    ...
    if (result && result[0] && result[0].result) {
      ...
      if (!(await isValidRecipePage(recipeData, tabUrl))) {
        return;
      }
...
async function sendCaptureMessage(tabId, tabUrl = "unknown") {
  ...
  const timeout = setTimeout(() => {
    if (!messageResponded) {
      console.error(
        "❌ Message timeout - Safari may need content script reload"
      );
  ...
  try {
    const response = await new Promise((resolve, reject) => {
      extensionAPI.tabs.sendMessage(
        ...
        (response) => {
          ...
          console.log(
            "🔧 Capture sendMessage callback - lastError:",
            extensionAPI.runtime.lastError
          );
          if (extensionAPI.runtime.lastError) {
            reject(extensionAPI.runtime.lastError);
    ...
    if (response && response.status === "success") {
      ...
      showStatus(
        "❌ Capture failed: " +
          (response ? response.message || "Invalid response" : "No response"),
        "#ffebee"
      );
    }
  } catch (error) {
  ...
  if (isAuthenticated) {
    ...
    try {
      ...
      if (devResult.success) {
        showStatus(
          "✅ Saved to development backend! Recipe ID: " + devResult.id,
          "#d4edda"
        );
      } else {
      ...
      showStatus(
        "❌ Error saving to development backend: " + error.message,
        "#ffebee"
      );
    }
  }
...
function transformRecipeDataForAWS(recipeData, currentUrl = null) {
  ...
  if (ingredients.length === 0) {
    ...
    ingredients.push({
      text: "[No ingredients extracted - parser debugging needed]",
    });
  }
...
async function sendToAWSBackend(recipeData, currentUrl = "unknown") {
  try {
    ...
    try {
      ...
      // Use the first available JWT token
      userToken = auth.idToken || auth.token || auth.accessToken;
    } catch {
      return {
        success: false,
    ...
    try {
      ...
      // Select the best image URL, prioritizing rawImageUrl from the content script
      let originalImageUrl =
        recipeData.rawImageUrl ||
        recipeData.imageUrl ||
      ...
      if (
        ...
        originalImageUrl = recipeData.photos[0];
      }
      ...
      if (originalImageUrl) {
        try {
          ...
          if (typeof s3ImageUrl === "string") {
            ...
            // Remove mainPhotoUrl to avoid validation errors
            delete transformedData.mainPhotoUrl;
          }
        } catch (imageError) {
          console.error("❌ Error processing image:", imageError);
          console.warn("⚠️ Image processing skipped due to upload failure");
          // Remove mainPhotoUrl to avoid validation errors
          delete transformedData.mainPhotoUrl;
        }
      }
      ...
      if (typeof submitDiagnosticData === "function") {
        ...
        typeof window.submitDiagnosticData === "function"
      ) {
      ...
      if (response.status === 401 && errorMessage.includes("expired")) {
        ...
        if (refreshResult.success) {
          ...
          if (newAuthData) {
            ...
            const newToken =
              newAuth.token || newAuth.accessToken || newAuth.idToken;
            ...
            if (retryResponse.ok) {
              ...
              return {
                success: true,
                id:
                  retryResult.id || retryResult.recipeId || "aws-" + Date.now(),
                result: retryResult,
    ...
    return {
      success: false,
      error: `AWS connection failed: ${_error.message}`,
    };
  }
}
