...
(() => {
  ...
  var BaseParser = class _BaseParser {
    ...
    /**
     * Split multi-step instructions into separate instruction objects
     * Handles cases where a single instruction contains multiple sentences or actions
     */
    splitInstructions(instructionText) {
      if (!instructionText) return [];
      const cleanText = this.sanitizeText(instructionText);
      const splitPatterns = [
        /\. (?=[A-Z])/g,
        // Period followed by capital letter
        /; (?=and |then |next |now |after |when )/gi,
        // Semicolon followed by transition words
        /\. (?=and |then |next |now |after |when )/gi
        // Period followed by transition words
      ];
      let parts = [cleanText];
      ...
      return parts.map((part) => part.trim()).filter((part) => part.length > 0).map((part) => {
        if (!/[.!?]$/.test(part)) {
          part += ".";
        }
        return part;
    ...
    /**
     * Process instruction array and split multi-step instructions
     */
    processInstructions(rawInstructions) {
      const instructions = [];
      let stepNumber = 1;
      for (const rawInstruction of rawInstructions) {
        const text3 = typeof rawInstruction === "string" ? rawInstruction : rawInstruction.text;
        const splitSteps = this.splitInstructions(text3);
    ...
    async parse(html3, url) {
      ...
      if (jsonLd) {
        recipe = {
          ...
          instructions: this.processInstructions(
            (jsonLd.recipeInstructions || []).map(
              (i) => this.extractInstructionText(i)
            )
          ),
        ...
        if (ingredientP.length) {
          ...
          if (raw) {
            ingredients = raw.split(/<br\s*\/>/i).map((t) => ({
              text: this.sanitizeText(
                $2(t).text() || $2("<div>" + t + "</div>").text()
              )
            })).filter((i) => i.text);
          }
        }
        ...
        let imageUrl;
        imageUrl = $2(".post-thumbnail-container img").first().attr("src");
        if (!imageUrl) {
          imageUrl = $2("img.wp-post-image").first().attr("src");
        }
        if (!imageUrl) {
          const entryContentImg = $2(".entry-content img").first().attr("src");
          if (entryContentImg && (entryContentImg.includes("/wp-content/uploads/") || entryContentImg.includes("smittenkitchen"))) {
        ...
        if (!imageUrl) {
          imageUrl = $2(".recipe-photo img, img[src*=\"wp-content/uploads\"]").first().attr("src");
        }
        ...
        recipe = {
          ...
          imageUrl: typeof imageUrl === "string" && imageUrl.trim().length > 0 ? imageUrl.trim() : void 0,
          prepTime,
          cookTime,
          totalTime,
          servings,
  ...
  var FoodNetworkParser = class extends BaseParser {
    ...
    async parse(html3, url) {
      ...
      const prepTime = this.sanitizeText(
        ...
        ).first().text() || $2(".recipe-meta-prep, .prep-time-value, [data-prep-time]").first().text() || $2("[aria-label*=\"prep\"], [title*=\"prep\"], .recipe-times .prep").first().text() || $2(".recipe-time:contains(\"prep\") ~ .time-value, .time-prep").first().text()
      );
      const cookTime = this.sanitizeText(
      ...
      const totalTime = this.sanitizeText(
        ...
        ).first().text() || $2(".recipe-meta-total, .total-time-value, [data-total-time]").first().text() || $2("[aria-label*=\"total\"], [title*=\"total\"], .recipe-times .total").first().text() || $2(".recipe-time:contains(\"total\") ~ .time-value, .time-total").first().text()
      );
      ...
      if (!validation.isValid) {
        throw new Error(
          `[FoodNetwork] Contract validation failed: ${JSON.stringify(validation)}`
        );
      }
      return recipe;
    }
  ...
  var AllRecipesParser = class extends BaseParser {
    ...
    async parse(html3, url) {
      ...
      const prepTime = this.sanitizeText(
        $2(".prep-time, .recipe-prep-time, [data-prep-time]").first().text() || $2(
          ".recipe-timing .prep-time, [data-testid=\"prep-time\"], .preparation-time"
        ).first().text() || $2("[aria-label*=\"prep\"], [title*=\"prep\"], .recipe-meta-prep").first().text()
      );
      const cookTime = this.sanitizeText(
      ...
      const totalTime = this.sanitizeText(
        $2(".total-time, .recipe-total-time, [data-total-time]").first().text() || $2(
          ".recipe-timing .total-time, [data-testid=\"total-time\"], .recipe-duration"
        ).first().text() || $2("[aria-label*=\"total\"], [title*=\"total\"], .recipe-meta-total").first().text()
      );
      const servings = this.sanitizeText(
        $2(".servings, .recipe-yield, .recipe-servings, [data-servings]").first().text() || $2(
        ...
        ).first().text() || $2(".recipe-facts .servings, .nutrition-info .servings").first().text()
      );
      const recipe = {
        title,
        source: url,
  ...
  var EpicuriousParser = class extends BaseParser {
    ...
    async parse(html3, url) {
      ...
      const prepTime = this.sanitizeText(
        $2(".prep-time, [data-testid=\"PrepTime\"]").first().text()
      );
      const cookTime = this.sanitizeText(
        $2(".cook-time, [data-testid=\"CookTime\"]").first().text()
      );
      const totalTime = this.sanitizeText(
        $2(".total-time, [data-testid=\"TotalTime\"]").first().text()
      );
      const servings = this.sanitizeText(
  ...
  var WashingtonPostParser = class extends BaseParser {
    canParse(url) {
      return url.includes("washingtonpost.com/food") || url.includes("washingtonpost.com") && url.includes("recipe") || url.includes("washingtonpost.com/news/voraciously") || url.includes("washingtonpost.com") && (url.includes("voraciously") || url.includes("food"));
    }
    async parse(html3, url) {
      const jsonLd = this.extractJsonLD(html3);
      if (jsonLd) {
      ...
      if (!validation.isValid) {
        throw new Error(
          `[WashingtonPost] Contract validation failed: ${JSON.stringify(validation)}`
        );
      }
      return recipe;
    }
    ...
    extractImage($2) {
      ...
      for (const selector of imageSelectors) {
        if (selector.startsWith("meta")) {
          ...
          if (src)
            return src.startsWith("http") ? src : `https://washingtonpost.com${src}`;
        }
      }
      return void 0;
    }
    ...
    parseFromJsonLD(jsonLd, url) {
      return {
        ...
        instructions: (jsonLd.recipeInstructions || []).map(
          (instruction, idx) => ({
            stepNumber: idx + 1,
  ...
  var AlexandrasKitchenParser = class extends BaseParser {
    ...
    async parse(html3, url) {
      ...
      try {
        ...
        console.log(
          "\u26A0\uFE0F Alexandra's Kitchen: JSON-LD failed, trying CSS selectors"
        );
    ...
    async parseWithSelectors($2, url) {
      try {
        const title = this.sanitizeText(
          $2("h1.entry-title, h1.post-title, .recipe-title h1, h1").first().text() || ""
        );
        if (!title) {
        ...
        const prepTime = this.sanitizeText(
          $2(".tasty-recipes-prep-time, .recipe-prep-time").first().text() || ""
        );
        const cookTime = this.sanitizeText(
          $2(".tasty-recipes-cook-time, .recipe-cook-time").first().text() || ""
        );
        const totalTime = this.sanitizeText(
          $2(".tasty-recipes-total-time, .recipe-total-time").first().text() || ""
        );
        const servings = this.sanitizeText(
          $2(".tasty-recipes-yield, .recipe-servings").first().text() || ""
        );
        let imageUrl = $2(
  ...
  // sites/lemonsandzest.ts
  var LemonsAndZestParser = class extends BaseParser {
    canParse(url) {
      return url.includes("lemonsandzest.com");
    }
    async parse(html3, url) {
      const $2 = load(html3);
      const jsonLd = this.extractJsonLD(html3);
