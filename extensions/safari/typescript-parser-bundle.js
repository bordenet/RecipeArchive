  ...
  var SmittenKitchenParser = class extends BaseParser {
    ...
    async parse(html3, url) {
      ...
      if (jsonLd) {
        ...
        let ingredients = [];
        const jetpackIngredients = $2(".jetpack-recipe-ingredients");
        if (jetpackIngredients.length > 0) {
          jetpackIngredients.children().each((_, el) => {
            ...
            if ($el.is("h5")) {
              ...
              if (headerText) {
                ingredients.push({ text: `## ${headerText}` });
              }
            } else if ($el.is("ul")) {
