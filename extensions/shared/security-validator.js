...
class SecurityValidator {
  ...
  validateIngredient(ingredient) {
    ...
    if (ingredient.length > 200) {
      return {
        valid: false,
        error: "Ingredient must be 200 characters or less",
      };
    }
  ...
  validateRecipe(recipe) {
    ...
    if (
      ...
      recipe.instructions.filter((i) => i && i.length > 0).length === 0
    )
      errors.push("At least one non-empty instruction is required");
    ...
    return {
      ...
      sanitized: {
        ...
        description: recipe.description
          ? this.sanitizeHTML(recipe.description)
          : "",
  ...
  validateDiagnosticData(data) {
    ...
    const sanitized = {
      ...
      diagnosticData: data.diagnosticData
        ? {
            ...data.diagnosticData,
            pageAnalysis: this.stripHTML(
              data.diagnosticData.pageAnalysis || ""
            ),
            extractionAttempts: this.stripHTML(
              data.diagnosticData.extractionAttempts || ""
            ),
            selectors: Array.isArray(data.diagnosticData.selectors)
              ? data.diagnosticData.selectors.map(this.stripHTML)
              : [],
          }
        : {},
    };
