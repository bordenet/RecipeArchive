...
describe("Parser Registry Integration Tests", () => {
  ...
  beforeAll(async () => {
    ...
    const bundlePath = path.join(
      __dirname,
      "../chrome/typescript-parser-bundle.js"
    );
  ...
  describe("Parser Registration and Detection", () => {
    test("Should detect Smitten Kitchen URLs", () => {
      const parser = registry.getParserForUrl(
        "https://smittenkitchen.com/2020/08/mathildes-tomato-tart/"
      );
    ...
    test("Should detect Food52 URLs", () => {
      const parser = registry.getParserForUrl(
        "https://food52.com/recipes/kylie-sakaida-avocado-bean-toast"
      );
      expect(parser).toBeDefined();
      expect(parser).not.toBeNull();
    });
  ...
  describe("Mock Recipe Parsing", () => {
    test("TypeScriptParser interface should be available", () => {
      ...
      expect(typeof window.TypeScriptParser.extractRecipeFromPage).toBe(
        "function"
      );
    });
    ...
    test("Should parse real Smitten Kitchen fixture correctly", async () => {
      ...
      const fixturePath = path.join(
        __dirname,
        "../../tests/fixtures/html-samples/smitten-kitchen-sample.html"
      );
      if (fs.existsSync(fixturePath)) {
        ...
        console.warn(
          "Smitten Kitchen fixture not found, skipping real parsing test"
        );
      }
    });
