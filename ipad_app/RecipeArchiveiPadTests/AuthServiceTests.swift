...
final class AuthServiceTests: XCTestCase {
    ...
    // MARK: - iPad-Specific Authentication Tests
    
    func testPersistentAuthenticationiPad() throws {
        // Test that authentication persists across app launches (important for iPad multitasking)
        let expectation = XCTestExpectation(description: "Persistent authentication")
        
        authService.signIn(email: "persistent@example.com", password: "password123")
    ...
    func testMultipleSignInAttemptsiPad() throws {
        // Test handling multiple rapid sign-in attempts (iPad users may tap multiple times)
        authService.signIn(email: "test@example.com", password: "password123")
        authService.signIn(email: "test@example.com", password: "password123")
        authService.signIn(email: "test@example.com", password: "password123")
        
        let expectation = XCTestExpectation(description: "Multiple sign in attempts")
        
        DispatchQueue.main.asyncAfter(deadline: .now() + 3.0) {
            // Should still only have one successful authentication
            XCTAssertTrue(self.authService.isAuthenticated)
            XCTAssertNotNil(self.authService.currentUser)
            expectation.fulfill()
        }
