...
export abstract class BaseParser {
   ...
   */
  protected splitInstructions(instructionText: string): string[] {
    if (!instructionText) return [];

    // Clean the text first
    const cleanText = this.sanitizeText(instructionText);

    // Split on periods followed by space and capital letter, or conjunctions that indicate new steps
    const splitPatterns = [
      /\. (?=[A-Z])/g, // Period followed by capital letter
      /; (?=and |then |next |now |after |when )/gi, // Semicolon followed by transition words
      /\. (?=and |then |next |now |after |when )/gi, // Period followed by transition words
    ];
   ...
   */
  protected processInstructions(
    rawInstructions: (string | { text: string })[]
  ): Instruction[] {
