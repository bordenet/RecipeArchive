...
export class EpicuriousParser extends BaseParser {
    ...
    async parse(html: string, url: string): Promise<Recipe> {
        ...
        if (!jsonLd) {
            throw new Error("[Epicurious] No JSON-LD recipe data found. Strict contract enforcement: no fallback.");
        }
        const recipe: Recipe = {
        ...
        if (!validation.isValid) {
            throw new Error(`[Epicurious] Contract validation failed: ${JSON.stringify(validation)}`);
        }
        return recipe;
    }
}
