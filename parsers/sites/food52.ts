...
export class Food52Parser extends BaseParser {
  ...
  async parse(html: string, url: string): Promise<Recipe> {
    ...
    const isLocalFixture =
      url.endsWith('.html') || url.startsWith('tests/fixtures/html-samples/');
    if (
      !isLocalFixture &&
      html.includes('Apologies, that page cannot be found.')
    ) {
    ...
    if (jsonLd) {
      ...
      if (jsonLd.recipeCategory) {
        ...
        if (Array.isArray(jsonLd.recipeCategory)) {
          tags.push(
            ...(jsonLd.recipeCategory as string[]).map((c: string) =>
              this.sanitizeText(c)
            )
          );
          ...
          tags.push(
            ...(jsonLd.recipeCategory as string)
              .split(/,|;/)
              .map((c: string) => this.sanitizeText(c))
              .filter(Boolean)
          );
        }
      }
