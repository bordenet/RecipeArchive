...
export class WashingtonPostParser extends BaseParser {
  ...
  private extractInstructions(
    $: cheerio.CheerioAPI
  ): Array<{ stepNumber: number; text: string }> {
  ...
  private extractIngredientsFromParagraphs(
    $: cheerio.CheerioAPI
  ): Array<{ text: string }> {
    const ingredients: Array<{ text: string }> = [];
  ...
  private extractInstructionsFromParagraphs(
    $: cheerio.CheerioAPI
  ): Array<{ stepNumber: number; text: string }> {
    const instructions: Array<{ stepNumber: number; text: string }> = [];
  ...
  private looksLikeIngredient(text: string): boolean {
    ...
    return (
      ingredientIndicators.some((pattern) => pattern.test(text)) &&
      text.length < 200
    );
  }
  ...
  private looksLikeInstruction(text: string): boolean {
    ...
    return (
      instructionIndicators.some((pattern) => pattern.test(text)) &&
      text.length > 20 &&
      text.length < 500
    );
  }
