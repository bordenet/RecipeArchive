...
export class AllRecipesParser extends BaseParser {
  ...
  async parse(html: string, url: string): Promise<Recipe> {
    ...
    const image =
      jsonLd.image?.url ||
      (typeof jsonLd.image === 'string' ? jsonLd.image : undefined) ||
    ...
    // Ingredients
    const ingredients = (jsonLd.recipeIngredient || []).map((i: string) => ({
      text: i,
    }));

    // Instructions
    const instructions = (jsonLd.recipeInstructions || []).map(
      (step: any, idx: number) => ({
        stepNumber: idx + 1,
    ...
    if (
      ...
      (jsonLd.aggregateRating.ratingValue || jsonLd.aggregateRating.reviewCount)
    ) {
      reviews =
    ...
    if (jsonLd.nutrition && typeof jsonLd.nutrition === 'object') {
      nutrition = Object.entries(jsonLd.nutrition)
        .map(([k, v]) => `${k}: ${v}`)
        .join('; ');
    }
