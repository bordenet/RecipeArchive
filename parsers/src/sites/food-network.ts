    ...
    if (jsonLd) {
      recipe = {
        ...
        instructions: (jsonLd.recipeInstructions || [])
          ...
          .filter(
            (inst) => typeof inst.text === 'string' && inst.text.length > 0
          ),
        image:
      ...
      $(
        '.o-Method__m-Step, .instructions-list__item, .instruction, .recipe-instructions li, ol li, ul li, .entry-content ol li, .entry-content ul li'
      ).each((i, el) => {
