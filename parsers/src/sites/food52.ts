...
class Food52Parser extends BaseParser {
  ...
  async parse(html: string, url: string): Promise<Recipe> {
    ...
    if (!author) {
      author = $('[class*="author"], .byline, .entry-author')
        .first()
        .text()
        .trim();
    }
    ...
    $('[class*="ingredient"], .recipe__ingredient, .ingredients li').each(
      (_, el) => {
        const text = $(el)
          .text()
          .trim()
    ...
    $('[class*="instruction"], .recipe__instruction, .instructions li').each(
      (_, el) => {
    ...
    if (
      ...
      instructions = jsonLd.recipeInstructions.map(
        (i: string | { text?: string }) =>
          typeof i === 'string' ? i.trim() : (i.text || '').trim()
      );
    }
    ...
    if (!imageUrl) {
      imageUrl =
        ...
        $('img.recipe__image, .main-image').attr('src') ||
        undefined;
    }

    // Times
    const prepTime =
      (jsonLd && jsonLd.prepTime) ||
      $('meta[itemprop="prepTime"]').attr('content') ||
      $('dt:contains("Prep Time")').next('dd').text().trim() ||
      '';
    const cookTime =
      (jsonLd && jsonLd.cookTime) ||
      $('meta[itemprop="cookTime"]').attr('content') ||
      $('dt:contains("Cook Time")').next('dd').text().trim() ||
      '';
    const totalTime =
      (jsonLd && jsonLd.totalTime) ||
      $('meta[itemprop="totalTime"]').attr('content') ||
      '';

    // Servings / Yield
    const servings =
      (jsonLd && jsonLd.recipeYield) ||
      $('[class*="servings"], .recipe__servings').first().text().trim() ||
      $('dt:contains("Serves")').next('dd').text().trim() ||
      '';
    ...
    const mappedInstructions = instructions.map((text, idx) => ({
      stepNumber: idx + 1,
      text,
    }));
