  ...
  async parse(html: string, url: string): Promise<Recipe> {
    ...
    function toIngredientList(arr: any): Ingredient[] {
      ...
      if (Array.isArray(arr))
        return arr.map((i: string) => ({ text: cleanString(i) }));
      return [];
    }
    function toInstructionList(arr: any): Instruction[] {
      ...
      if (Array.isArray(arr))
        return arr.map((i: any, idx: number) => ({
          stepNumber: idx + 1,
          text: cleanString(typeof i === 'string' ? i : i.text),
        }));
      return [];
    }
    ...
    let title = jsonLdRecipe
      ? toString(jsonLdRecipe.name)
      : $('h1.entry-title').text().trim() ||
        $('h1').first().text().trim() ||
        $('meta[property="og:title"]').attr('content') ||
        '';
    let author = jsonLdRecipe
      ? toString(
          jsonLdRecipe.author && jsonLdRecipe.author.name
            ? jsonLdRecipe.author.name
            : jsonLdRecipe.author
        )
      : $('.author').text().trim() ||
        $('meta[name="author"]').attr('content') ||
        'Jeanine Donofrio';
    let description = jsonLdRecipe
      ? toString(jsonLdRecipe.description)
      : $('meta[name="description"]').attr('content') ||
        cleanString($('.first-sentence').text()) ||
        cleanString($('.entry-content p').first().text()) ||
        '';
    let categories = jsonLdRecipe ? toList(jsonLdRecipe.recipeCategory) : [];
    let ingredients = jsonLdRecipe
      ? toIngredientList(jsonLdRecipe.recipeIngredient)
      : [];
    let instructions = jsonLdRecipe
      ? toInstructionList(jsonLdRecipe.recipeInstructions)
      : [];
    let image = jsonLdRecipe
      ? toString(jsonLdRecipe.image)
    ...
    let reviews =
      jsonLdRecipe && jsonLdRecipe.aggregateRating
        ? jsonLdRecipe.aggregateRating.ratingValue ||
          jsonLdRecipe.aggregateRating.reviewCount ||
          ''
        : '';
    let nutrition =
      jsonLdRecipe && jsonLdRecipe.nutrition
        ? JSON.stringify(jsonLdRecipe.nutrition)
        : '';
