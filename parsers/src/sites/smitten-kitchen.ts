  ...
  async parse(html: string, url: string): Promise<Recipe> {
    ...
    if (jsonLd) {
      recipe = {
        ...
        categories: Array.isArray(jsonLd.recipeCategory)
      ...
      let author = this.sanitizeText(
        $('.jetpack-recipe-source, p.recipe-meta + p, .author-meta, .author')
          .first()
          .text()
      ...
      if (jetpackDirections) {
        instructions.push({
          stepNumber: 1,
          text: this.sanitizeText(jetpackDirections),
        });
      }
      ...
      // Improved image extraction with prioritized selectors for Smitten Kitchen
      let imageUrl = '';

      // Priority 1: Main post thumbnail (most reliable for Smitten Kitchen)
      imageUrl = $('.post-thumbnail-container img').first().attr('src') || '';

      // Priority 2: WordPress post image class
      if (!imageUrl) {
        imageUrl = $('img.wp-post-image').first().attr('src') || '';
      }

      // Priority 3: Entry content images with recipe-related paths
      if (!imageUrl) {
      ...
      // Priority 4: Open Graph image fallback
      if (!imageUrl) {
        imageUrl = $('meta[property="og:image"]').attr('content') || '';
      }

      // Priority 5: Any reasonable image as last resort
      if (!imageUrl) {
        imageUrl = $('.recipe-photo img, img').first().attr('src') || '';
      }
      ...
      let description =
        this.sanitizeText(
          $('.jetpack-recipe-notes, .entry-content p').first().text()
        ) || '';
      if (!description) {
        description =
          this.sanitizeText(
            $('.entry-content').first().text().split('\n')[0]
          ) || '';
      }
    ...
    // Lenient validation with fallbacks - only require truly essential fields
    ...
    if (!recipe.source_url || recipe.source_url.trim().length === 0) {
      recipe.source_url = url || 'https://smittenkitchen.com/';
    }

    // Provide fallback for missing ingredients
    if (!recipe.ingredients || recipe.ingredients.length === 0) {
      recipe.ingredients = [{ text: 'See original recipe for ingredients.' }];
    }

    // Provide fallback for missing instructions
    ...
    if (!recipe.description) {
      recipe.description = 'A delicious recipe from Smitten Kitchen.';
    }
    ...
    if (!recipe.image) {
      recipe.image = ''; // Empty string is acceptable for image
    }

    if (!recipe.yield) {
      recipe.yield = 'See recipe';
    }
    ...
    if (!recipe.nutrition) {
      recipe.nutrition = 'See original recipe for nutrition information.';
    }
