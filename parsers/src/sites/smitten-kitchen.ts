...
export class SmittenKitchenParser extends BaseParser {
	...
	async parse(html: string, url: string): Promise<Recipe> {
		...
		if (jsonLd) {
			...
			let title = this.sanitizeText($('h1.entry-title, h1.post-title, h1').first().text() || $('h1').first().text() || "");
			let author = this.sanitizeText($('p.recipe-meta + p, .author-meta, .author').first().text().replace('Author:', '').trim() || 'Deb Perelman');
