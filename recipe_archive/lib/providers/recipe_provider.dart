...
class RecipeNotifier extends StateNotifier<AsyncValue<List<Recipe>>> {
  ...
  RecipeNotifier(this._recipeService, this._analyticsService) : super(const AsyncValue.loading()) {
    loadRecipes();
  }
  ...
  Future<void> searchRecipes(String query) async {
    try {
      ...
      final responseTimeMs = searchEnd.difference(searchStart).inMilliseconds;

      state = AsyncValue.data(recipes);

      // Track analytics for the search - only for meaningful queries
        ...
        try {
          await _analyticsService.trackSearch(
            ...
            searchParams: SearchParameters(
              query: query,
              limit: 50, // Match the backend default
              sortBy: 'createdAt',
              sortOrder: 'desc',
            ),
          );
...
final recipeProvider = StateNotifierProvider<RecipeNotifier, AsyncValue<List<Recipe>>>((ref) {
  ...
  final analyticsService = ref.watch(unifiedAnalyticsServiceProvider);
  return RecipeNotifier(recipeService, analyticsService);
});
...
// Filtered recipes provider - now uses server-side search with analytics
final filteredRecipesProvider = Provider<AsyncValue<List<Recipe>>>((ref) {
  final recipes = ref.watch(recipeProvider);
  final searchQuery = ref.watch(searchQueryProvider);
  final selectedCategory = ref.watch(selectedCategoryProvider);

  // If there's a search query, use server-side search with analytics
  if (searchQuery.isNotEmpty) {
    // Trigger server-side search which includes analytics tracking
    Future.microtask(() {
      ref.read(recipeProvider.notifier).searchRecipes(searchQuery);
    });
  }
  ...
  return recipes.when(
    data: (recipeList) {
      ...
      // Apply category filter (search filtering is now handled by server)
