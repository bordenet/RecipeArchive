...
class _RecipeEditScreenState extends ConsumerState<RecipeEditScreen> with TickerProviderStateMixin {
  ...
  List<String> _semanticTags = [];
  List<String> _primaryIngredients = [];
  List<String> _cookingMethods = [];
  List<String> _dietaryTags = [];
  List<String> _flavorProfile = [];
  ...
  void _loadRecipeData() {
    ...
    _semanticTags = List.from(widget.recipe.semanticTags);
    _primaryIngredients = List.from(widget.recipe.primaryIngredients);
    _cookingMethods = List.from(widget.recipe.cookingMethodsTags);
    _dietaryTags = List.from(widget.recipe.dietaryTags);
    _flavorProfile = List.from(widget.recipe.flavorProfile);
    _equipment = List.from(widget.recipe.equipment);
    _timeCategory = widget.recipe.timeCategory ?? '';
    _complexity = widget.recipe.complexity ?? '';
    _mealType = List.from(widget.recipe.mealType);
              ...
              Expanded(
                child: TextFormField(
                  ...
                  decoration: InputDecoration(
                    labelText: '${_mealType.contains('drink') ? 'Mix' : 'Cook'} Time (minutes)',
  ...
  Future<void> _saveRecipe() async {
    ...
    try {
      ...
      final updatedRecipe = widget.recipe.copyWith(
        ...
        semanticTags: _semanticTags,
        primaryIngredients: _primaryIngredients,
        cookingMethodsTags: _cookingMethods,
        dietaryTags: _dietaryTags,
        flavorProfile: _flavorProfile,
        equipment: _equipment,
        timeCategory: _timeCategory.isNotEmpty ? _timeCategory : null,
        complexity: _complexity.isNotEmpty ? _complexity : null,
        mealType: _mealType,
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          Text(
            'These tags help others find your recipe when searching',
          ...
          _buildSearchTagSection('Meal Type', _mealType, [
            'breakfast', 'lunch', 'brunch', 'dinner', 'snack', 'dessert', 'appetizer', 'drinks'
          ]),
          const SizedBox(height: 20),
          
          _buildSearchTagSection('Semantic Tags', _semanticTags, [
            'italian', 'comfort-food', 'weeknight', 'healthy', 'quick', 'family-friendly',
            'vegetarian', 'spicy', 'sweet', 'savory', 'holiday', 'summer', 'winter'
          ]),
          const SizedBox(height: 20),
          
          _buildSearchTagSection('Primary Ingredients', _primaryIngredients, [
            'chicken', 'beef', 'pork', 'fish', 'vegetables', 'pasta', 'rice', 'potatoes',
            'tomatoes', 'cheese', 'eggs', 'beans', 'mushrooms', 'garlic', 'onions'
          ]),
          const SizedBox(height: 20),
          
          _buildSearchTagSection('Cooking Methods', _cookingMethods, [
            'baked', 'grilled', 'sautéed', 'roasted', 'slow-cooked', 'fried', 'steamed',
            'braised', 'boiled', 'no-cook', 'one-pot', 'sheet-pan'
          ]),
          const SizedBox(height: 20),
          
          _buildSearchTagSection('Dietary Tags', _dietaryTags, [
            'vegetarian', 'vegan', 'gluten-free', 'dairy-free', 'low-carb', 'keto',
            'paleo', 'low-sodium', 'sugar-free', 'nut-free'
          ]),
          const SizedBox(height: 20),
          
          _buildSearchTagSection('Flavor Profile', _flavorProfile, [
            'savory', 'sweet', 'spicy', 'herbed', 'tangy', 'rich', 'fresh', 'smoky', 'creamy'
          ]),
          const SizedBox(height: 20),
          
          _buildSearchTagSection('Equipment', _equipment, [
            'oven', 'stovetop', 'slow-cooker', 'grill', 'instant-pot', 'food-processor',
            'blender', 'stand-mixer', 'large-pot', 'sheet-pan'
          ]),
          const SizedBox(height: 20),
          
          _buildDropdownSection('Time Category', _timeCategory, [
            'quick-15min', 'medium-30min', 'long-60min'
          ]),
          const SizedBox(height: 20),
          
          _buildDropdownSection('Complexity', _complexity, [
  ...
  Widget _buildSearchTagSection(String title, List<String> selectedTags, List<String> availableTags) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
  ...
  Widget _buildDropdownSection(String title, String selectedValue, List<String> options) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(title, style: Theme.of(context).textTheme.titleMedium),
