...
import 'backup_service_web.dart' if (dart.library.io) 'backup_service_stub.dart' as web_service;
...
class BackupService {
  ...
  Future<BackupResult> exportRecipes() async {
    try {
      ...
      // Get app version info with web fallback
      ...
      try {
        ...
        // For web, use default values since package_info_plus isn't properly registered
      } catch (e) {
        // Fallback to default values if package_info fails
      }
      ...
      if (kIsWeb) {
        return await web_service.exportForWeb(jsonString);
      } else {
        return await _exportForMobile(jsonString);
      }
    } catch (e) {
      String errorMessage = 'Failed to export recipes: $e';
      
      // Check for authentication errors
      if (e.toString().contains('401') || e.toString().contains('Unauthorized')) {
        errorMessage = 'Authentication expired. Please sign in again and try exporting.';
      } else if (e.toString().contains('Network error')) {
        errorMessage = 'Network connection failed. Please check your internet connection and try again.';
      }
      
      return BackupResult.error(errorMessage);
    }
  }
  ...
  Future<BackupResult> _exportForMobile(String jsonContent) async {
    try {
      ...
      // Share the file using the SharePlus instance API
