...
class QualityGate {
  ...
  async checkFlutterAnalyze() {
    ...
    try {
      ...
      // Ensure Flutter .env asset exists so analyzer sees the declared asset path
      const flutterEnvPath = "recipe_archive/.env";
      if (!this.fileExists(flutterEnvPath)) {
        const placeholderEnv = [
          "# Generated by mcp-diagnostics-gate for analysis only",
          "# Do NOT store real secrets here; use the root .env for real config.",
          "API_BASE_URL=https://example.execute-api.us-west-2.amazonaws.com/prod",
          "COGNITO_USER_POOL_ID=us-west-2_EXAMPLEPOOL",
          "COGNITO_CLIENT_ID=EXAMPLECLIENTID1234567890",
          ""
        ].join("\n");
        writeFileSync(flutterEnvPath, placeholderEnv, { encoding: "utf8" });
        this.log(
          YELLOW,
          "⚠️  Created placeholder recipe_archive/.env for Flutter analyze (no real secrets)"
        );
      }
