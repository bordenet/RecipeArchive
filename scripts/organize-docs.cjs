...
class DocOrganizer {
  ...
  ensureDirectoryExists(dirPath) {
    if (!fs.existsSync(dirPath)) {
      ...
      console.log(
        `📁 Created directory: ${path.relative(this.rootDir, dirPath)}`
      );
    }
  }
  ...
  moveFile(sourcePath, targetPath) {
    try {
      ...
      console.log(
        `📋 Moved: ${path.basename(sourcePath)} → ${path.relative(this.rootDir, targetPath)}`
      );
      return true;
    } catch (error) {
      console.error(
        `❌ Failed to move ${path.basename(sourcePath)}: ${error.message}`
      );
      return false;
    }
  }
  ...
  organizeDocumentation() {
    ...
    console.log(
      `   ✅ Root directory: ${scannedCount - movedCount} appropriate files remaining`
    );
  ...
  organizeConfigFiles() {
    ...
    console.log(
      `   ✅ Root directory: ${configFiles.length - movedCount} essential configs remaining`
    );
