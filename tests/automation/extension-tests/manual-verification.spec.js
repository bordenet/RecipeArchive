...
test.describe('Chrome Extension Manual Verification', () => {
  test('Verify extension loads in Chrome', async ({ page }) => {
    ...
    const testPagePath =
      'file://' +
      path.resolve(__dirname, '../../../tests/chrome-extension-test.html');
    ...
    await page.screenshot({
      path: 'chrome-extension-test-page.png',
      fullPage: true,
    });
    ...
    console.log(
      '💡 Check Chrome manually for extension loading and functionality'
    );
  });
  ...
  test('Verify mock server is accessible', async ({ page }) => {
    ...
    try {
      ...
      console.log(
        '💡 Start the server: cd aws-backend/functions/local-server && go run main.go'
      );
    }
  });
});
