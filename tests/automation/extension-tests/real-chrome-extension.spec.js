...
const REAL_EXTENSION_PATH = path.resolve(
  __dirname,
  '../../../extensions/chrome'
);
...
test.describe('Real Chrome Extension E2E Tests', () => {
  ...
  test('02. Check extension loading and background pages', async () => {
    ...
    if (backgroundPages.length === 0) {
      ...
      await page.screenshot({
        path: 'test-results/extension-state.png',
        fullPage: true,
      });
  ...
  test('05. Debug extension errors', async () => {
    ...
    const errorElements = page.locator(
      '.error-message, .warning-message, [data-test-id*="error"]'
    );
    ...
    await page.screenshot({
      path: 'test-results/extensions-debug.png',
      fullPage: true,
    });
