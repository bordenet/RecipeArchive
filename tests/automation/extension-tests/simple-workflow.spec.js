  ...
  test('Complete extension workflow', async () => {
    ...
    try {
      ...
      if (!title.includes('Simple White Cake')) {
        console.log(
          "⚠️ Warning: Page title doesn't contain expected recipe name"
        );
      } else {
    ...
    try {
      ...
      for (let i = 0; i < count; i++) {
        ...
        if (
          name &&
          (name.includes('RecipeArchive') || name.includes('Recipe Archive'))
        ) {
      ...
      if (!extensionId) {
        throw new Error(
          'RecipeArchive extension not found. Please ensure it is loaded.'
        );
      }
      ...
      if (await captureBtn.isVisible()) {
        ...
        try {
          ...
          if (response.status() === 201) {
            ...
            console.log(
              '❌ Recipe capture failed with status:',
              response.status()
            );
      ...
      try {
        ...
        console.log(
          '⚠️ Failed to fetch recipes from server:',
          serverError.message
        );
      }
