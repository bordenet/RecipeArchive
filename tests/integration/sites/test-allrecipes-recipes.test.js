...
const ALLRECIPES_RECIPES = [
  {
    url: 'https://www.allrecipes.com/recipe/17481/simple-white-cake/',
    expected: 'Simple White Cake',
    category: 'dessert',
  },
];

describe('AllRecipes Recipe Extraction', () => {
  let browser;
  let context;
...
async function testRecipeExtraction(page, recipe) {
  const result = await page.evaluate(() => {
    // JSON-LD Extraction (Primary method for AllRecipes)
    function extractRecipeFromJsonLd() {
      ...
      for (const script of jsonLdScripts) {
        try {
          ...
          console.log('AllRecipes: JSON-LD parsing failed:', e.message);
        }
      }
      return null;
    }
    // Fallback: AllRecipes HTML parsing (if needed)
    function extractAllRecipesRecipe() {
      const title = document.querySelector('h1.headline')?.textContent?.trim() || document.title || 'Unknown Recipe';
      let ingredients = [];
      const ingredientSelectors = ['.ingredients-item-name'];
      ...
      let instructions = [];
      const stepSelectors = ['.instructions-section li p'];
