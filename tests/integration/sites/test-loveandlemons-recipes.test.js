...
const LOVEANDLEMONS_RECIPES = [
  {
    url: 'https://www.loveandlemons.com/corn-salsa/',
    expected: 'Corn Salsa',
    category: 'side',
  },
  {
    url: 'https://www.loveandlemons.com/grilled-corn-on-the-cob/',
    expected: 'Grilled Corn on the Cob',
    category: 'side',
  },
];

describe('Love and Lemons Recipe Extraction', () => {
  let browser;
  let context;
  ...
  test.each(LOVEANDLEMONS_RECIPES)(
...
async function testRecipeExtraction(page, recipe) {
  const result = await page.evaluate(() => {
    // JSON-LD Extraction (Primary method for Love and Lemons)
    function extractRecipeFromJsonLd() {
      ...
      for (const script of jsonLdScripts) {
        try {
          ...
          console.log('Love and Lemons: JSON-LD parsing failed:', e.message);
        }
      }
      return null;
    }
    // Fallback: Love and Lemons HTML parsing (if needed)
    function extractLoveAndLemonsRecipe() {
      const title = document.querySelector('h1.entry-title')?.textContent?.trim() || document.title || 'Unknown Recipe';
      let ingredients = [];
      const ingredientSelectors = ['.wprm-recipe-ingredient'];
      ...
      let instructions = [];
      const stepSelectors = ['.wprm-recipe-instruction-text'];
    ...
    return extractLoveAndLemonsRecipe();
  });
  return result;
}
