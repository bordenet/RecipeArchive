...
async function testSafariFocacciaFix() {
  ...
  try {
    ...
    const result = await page.evaluate(() => {
      ...
      try {
        ...
        return {
          ...
          stepCount:
            data.steps?.reduce((sum, s) => sum + (s.items?.length || 0), 0) ||
            0,
          source: data.source,
        };
      } catch (error) {
    ...
    console.log(
      `🥘 Ingredients Found: ${result.hasIngredients} (${result.ingredientCount} items)`
    );
    console.log(
      `📋 Steps Found: ${result.hasSteps} (${result.stepCount} items)`
    );
    ...
    const testPassed =
      result.success &&
      result.hasTitle &&
      (result.hasIngredients || result.hasSteps);
