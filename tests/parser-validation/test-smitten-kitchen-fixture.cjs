// Contract validation test for Smitten Kitchen parser using static fixture
const fs = require('fs');
const path = require('path');
const { SmittenKitchenParser } = require('../../parsers/sites/smitten-kitchen');

const FIXTURE_PATH = path.join(__dirname, '../fixtures/html-samples/smitten-kitchen-sample.html');
const fixtureHtml = fs.readFileSync(FIXTURE_PATH, 'utf8');

(async () => {
  const parser = new SmittenKitchenParser();
  try {
