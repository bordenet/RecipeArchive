...
#
# This script tests the complete pipeline: Recipe Creation -> Background
# Normalization -> Search Enhancement. It validates that the search metadata is
# correctly generated and integrated, that the normalization trigger is in place,
# and that the search functionality works with normalized data.
#
# USAGE:
#   ./search-integration.sh
#
# ENVIRONMENT VARIABLES:
#   - S3_STORAGE_BUCKET: The S3 bucket used for storage.
#
# NOTES:
#   - This script is designed to be run from the root of the monorepo.
#   - It requires aws-cli, python3 or node to be installed.
#
################################################################################
...
# Test configuration
if [ -z "$S3_STORAGE_BUCKET" ]; then
    echo "❌ Missing required environment variable: S3_STORAGE_BUCKET"
...
TEST_TOOLS_PATH="$SCRIPT_DIR/../aws-backend/functions/test-tools/test-tools"
...
test_search_metadata_integration() {
    ...
    if [ -f "$TEST_TOOLS_PATH" ]; then
        local recipe_data=$(S3_STORAGE_BUCKET="$S3_BUCKET" "$TEST_TOOLS_PATH" -action=get-recipe -recipe-id=be18e0d4-a6e0-4007-ae65-5a553c06f0b2 2> /tmp/search-integration-curl.log | head -50)
    ...
    if command -v python3 &> /dev/null && echo "$sample_json" | python3 -m json.tool > /dev/null 2>&1; then
        print_success
        echo "    SearchMetadata JSON structure is valid"
    elif command -v node &> /dev/null && echo "$sample_json" | node -p "JSON.stringify(JSON.parse(require('fs').readFileSync(0)))" > /dev/null 2>&1; then
        print_success
        echo "    SearchMetadata JSON structure is valid"  
    else
...
test_search_with_normalized_data() {
    ...
    if [ -f "$TEST_TOOLS_PATH" ]; then
        local recipe_count=$(S3_STORAGE_BUCKET="$S3_BUCKET" "$TEST_TOOLS_PATH" -action=list-recipes -user-id="$TEST_USER_ID" 2> /tmp/search-integration-curl.log | grep -c "Recipe ID:" || echo "0")
    ...
    if command -v aws > /dev/null 2>&1; then
        local s3_object_count=$(aws s3 ls "s3://$S3_BUCKET/recipes/$TEST_USER_ID/" --region us-west-2 2> /tmp/search-integration-curl.log | wc -l || echo "0")
...
test_end_to_end_pipeline() {
    ...
    if [ -f "aws-backend/functions/recipes/main.go" ]; then
        ...
        if grep -q "Max [0-9]" "aws-backend/functions/models/recipe.go" 2> /tmp/search-integration-curl.log; then
            cost_features=$((cost_features + 1))
        fi
