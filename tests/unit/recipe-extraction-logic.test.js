...
function testAllRecipesTypeArrayHandling() {
  ...
  deepEqual(
    result.rawType,
    ["Recipe", "NewsArticle"],
    "Should have correct @type array"
  );
...
function testAllRecipesImageObjectExtraction() {
  ...
  assert(
    result.rawImageData["@type"] === "ImageObject",
    "Should preserve ImageObject structure"
  );
...
function testAllRecipesDataExtraction() {
  ...
  assert(
    result.ingredients[0] === "2 1/3 cups all-purpose flour",
    "Should extract first ingredient"
  );

  assert(result.steps.length === 2, "Should extract 2 steps");
  assert(
    result.steps[0] === "Preheat oven to 350 degrees F (175 degrees C).",
    "Should extract first step"
  );

  deepEqual(
    result.servingSize,
    ["1 cake", "8 servings"],
    "Should extract serving size array"
  );
...
function testTraditionalRecipe() {
  ...
  assert(
    result.title === "Traditional Chocolate Chip Cookies",
    "Should extract title"
  );
  assert(result.rawType === "Recipe", "Should preserve string @type");
  assert(result.photos.length === 1, "Should extract photo URL");
  assert(
    result.photos[0] === "https://example.com/cookies.jpg",
    "Should extract direct URL"
  );
...
function testInvalidData() {
  ...
  assert(
    extractRecipeFromJsonLd({ "@type": "Article" }) === null,
    "Should ignore non-Recipe types"
  );
  ...
  assert(
    extractRecipeFromJsonLd(nonRecipeData) === null,
    "Should ignore non-Recipe array types"
  );
