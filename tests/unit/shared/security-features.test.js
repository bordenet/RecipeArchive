...
describe("Security Features", () => {
  ...
  describe("Input Sanitization", () => {
    test("sanitizes malicious HTML from recipe titles", () => {
      const maliciousTitle =
        "<script>alert(\"xss\")</script>Chocolate Cake<img src=x onerror=alert(1)>";
    ...
    test("validates recipe ingredients and removes HTML", () => {
      ...
      const result =
        securityValidator.validateIngredients(maliciousIngredients);
  ...
  describe("JWT Validation", () => {
    ...
    test("extracts Cognito user info correctly", () => {
      ...
      const mockIdToken =
        ...
        btoa(JSON.stringify(cognitoPayload)) +
        ".signature";
  ...
  describe("Diagnostic Data Validation", () => {
    test("sanitizes diagnostic data to prevent injection", () => {
      ...
      const result = securityValidator.validateDiagnosticData(
        maliciousDiagnosticData
      );

      expect(result.valid).toBe(true);
      expect(result.sanitized.userAgent).toBe("Mozilla/5.0");
      expect(result.sanitized.diagnosticData.pageAnalysis).toBe(
        "Recipe analysis"
      );
      expect(result.sanitized.diagnosticData.extractionAttempts).not.toContain(
        "<script>"
      );
    });
